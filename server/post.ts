...
import { defaultPostTitle, maxPostTitleLength, maxPostExcerptLength } from '@/common/post';
import { shortenText } from '@/server/text';
...
export const extractMeta = (bodyJson: string): { title: string; excerpt: string; slug: string } => {
  ...
  const slug = toSlug(`${title}-${seed}`, { lower: false });
